...
export type WorkoutBuilderProps = {
  ...
  /**
   * Set when the builder is inside a dialog. It then reports the saved workout
   * instead of navigating, and drops the page header — the dialog already has
   * a title and a way out.
   */
  embedded?: boolean;
  onSaved?: (workoutId: string) => void;
  onCancel?: () => void;
  /** The coach hands a workout over through localStorage; the URL only flags it. */
  fromCoach?: boolean;
};
...
export function WorkoutBuilder({
  ...
  const handleBack = () => {
    if (hasUnsavedChanges) {
      setPendingNavigation(() => () => (embedded ? onCancel?.() : router.back()));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let error;
      let savedId: string | null = state.workoutId ?? null;
      if (state.mode === "edit" && state.workoutId) {
        ...
        // Create new workout. The id is selected back because an embedded
        // builder has to hand it to whoever opened it — a plan day cannot use
        // a workout it cannot name.
        const result = await supabase.from("workouts").insert(workoutData).select("id").single();
        error = result.error;
        savedId = (result.data?.id as string | undefined) ?? null;
      }
  ...
  return (
    <div className={embedded ? "pb-2" : "max-w-6xl mx-auto pb-12"}>
      {/* Header. In a dialog the title and the way out are the dialog's job. */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0 flex items-center gap-2">
            <h1 className={embedded ? "sr-only" : "text-xl sm:text-2xl font-bold truncate"}>
